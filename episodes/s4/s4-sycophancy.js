...
export default {
  ...
  subtitle: '아첨과 편향',
  summary: '"이 수업안 괜찮지?"엔 "훌륭해요!", "별로지?"엔 "맞아요, 문제가 있어요". AI가 왜 내 편을 들어주기 쉬운지, 편향은 또 뭔지 짚어봤어요.',
  ...
  scenes: [
    ...
    },
    {
      ...
      captions: [
        ...
        { t: 9.5, text: '정도는 학습 자료를 <em>어떻게 골라 다듬었느냐</em>에 달려요.' }
      ],
      build({ stage, lines, P, tl }) {
        ...
        const chip = P.chip({ x: 330, y: 470, text: '정도만 달라요', color: 'aqua', size: 24 });
        tl.at(stage.appendChild(chip.el), 9.6, { from: 'pop' });
        const b = P.bubble({ x: 200, y: 560, w: 560, text: '그래도 늘 확인은 필요해요.', tail: 'left', size: 22, tone: 'soft' });
    ...
    },
    {
      ...
      captions: [
        ...
        { t: 9.3, text: '반대 의견도 같이 물어보면, 답이 한쪽으로 쏠리는 걸 줄일 수 있어요.' }
      ],
      build({ stage, lines, P, tl }) {
        const rows = [
          ['의견 빼고 중립으로 묻기', 110],
...
편향은 하나 더 있어요. 학습 자료에 담긴 직업이나 성별 같은 고정관념도 답에 묻어날 수 있어요. 정도는 학습 자료를 어떻게 골라 다듬었느냐에 달려 있어요.

그러니 의견은 빼고 중립으로 묻고, "반대 근거 3개"처럼 반론을 시켜 보세요. 같은 질문을 찬성 프레임과 반대 프레임으로 두 번 물어보면 흔들림이 눈에 보여요. 반대 의견도 같이 물어보는 습관, 오늘부터 들이면 좋겠어요.
`
};
