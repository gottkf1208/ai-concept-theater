...
export default {
  ...
  scenes: [
    ...
    },
    {
      ...
      captions: [
        ...
        { t: 9.5, text: '개발자용 API에서는 <em>system</em> 메시지로 들어가고, 앱에서는 지침·맞춤 설정이라는 이름으로 보여요.' }
      ],
      build({ stage, lines, P, tl }) {
        ...
        const names = P.text({ x: 340, y: 470, w: 820, text: 'API에서는 <em>system</em> 메시지, 앱에서는 <em>지침</em>·<em>맞춤 설정</em>이라는 이름으로 보여요.', size: 24, weight: 700 });
  ...
  tip: {
    ...
    extra: '개발자용 API에서는 system 메시지, 앱에서는 지침이나 맞춤 설정이라는 이름으로 같은 자리를 가리켜요.'
  },
  myth: {
...
이렇게 대화 맨 앞에 놓이는 지시문을 시스템 프롬프트라고 불러요. 역할과 독자, 형식, 금지 사항을 적어 두면 이후 모든 답에 영향을 줘요. API에서는 system 메시지, 앱에서는 지침이나 맞춤 설정이라는 이름으로 같은 자리를 가리켜요.
