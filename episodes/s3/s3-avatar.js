...
export default {
  ...
  scenes: [
    {
      ...
      captions: [
        { t: 0, text: '오늘 한 선생님이 AI 아바타가 강의하는 영상을 보고 깜짝 놀랐어요.' },
    ...
    },
    {
      ...
      captions: [
        ...
        { t: 9, text: '단서를 훈련받아도 <em>65.1%</em> 정도예요(Diel 외 2024 메타분석).' }
      ],
      build({ stage, lines, P, tl }) {
    ...
    },
    {
      ...
      captions: [
        ...
        { t: 5, text: '<em>손·소품</em>, <em>배경 경계</em>, <em>소리와 입의 어긋남</em>도 단서가 돼요.' },
        { t: 10, text: '다만 AI 모델의 "AI가 만든 이미지인지" 판정도 믿을 만하지 않아요. Claude 문서는 판정할 수 없다고 명시해요.' }
      ],
      build({ stage, lines, P, tl }) {
        const chipDefs = [
        ...
        const cite = P.box({ x: 190, y: 380, w: 950, h: 110, label: 'AI도 스스로는 몰라요', sub: '"AI가 만든 이미지인지" 판정하지 못한다고 공식 문서가 밝혀요 (Claude Vision 문서)', accent: 'ink' });
    ...
    },
    {
      title: '할 일', dur: 12,
      captions: [
        { t: 0, text: '오늘 기억할 세 가지예요.' },
        { t: 4, text: '<em>진짜 같음 ≠ 진짜</em>. 그리고 출처(누가, 어디에 올렸나)부터 봐요.' },
  ...
  script: `
오늘 한 선생님이 AI 아바타 강의 영상에 놀랐대요. 입 모양이 소리에 딱 맞아 진짜 같았거든요. 성인 실험에서 아바타 영상과 강사 영상의 학습 효과는 차이가 없었어요(Leiker 외 2023). 설명은 영상에, 판단과 관계는 교사가 맡아요.

원리는 간단해요. 소리로 입 모양을 예측해 얼굴 영상에 입만 바꿔 붙여요. Wav2Lip처럼 립싱크 판별기와 겨루며 학습해요.

문제는 사람 눈으로 가려내기 어렵다는 거예요. 정확도는 평균 55.5퍼센트, 동전 던지기 수준이에요. 훈련 후에도 65.1퍼센트예요(Diel 외 2024).

단서는 있어요. 입 안쪽·치아, 눈 깜빡임, 손·소품, 배경 경계, 소리와 입의 어긋남이요. 다만 AI도 'AI 생성 여부'는 스스로 판정 못해요.

학생에게는 이렇게 말해 주세요. 진짜 같다고 진짜는 아니에요. 누가 올렸는지부터 보고, 학교 영상엔 AI라고 표시해요.
`
};
