/* E1 [B] AI는 어떻게 브루를 직접 눌렀을까: AI 에이전트와 MCP (파일럿) */
export default {
  slug: 'e1-agent-mcp',
  track: 'B',
  title: 'AI는 어떻게 브루를 직접 눌렀을까',
  subtitle: 'AI 에이전트와 MCP',
  summary: 'Claude가 크롬 확장으로 브루 웹을 직접 눌러 영상 6편을 만든 날. 요청→계획→도구→관찰의 고리와, AI와 도구를 잇는 규격 MCP를 3분에.',
  keywords: ['에이전트', 'agent', 'MCP', '도구', '브루', '자동화', '승인'],

  scenes: [
    {
      title: '오늘 있었던 일', dur: 13,
      captions: [
        { t: 0, text: '오늘 Claude가 크롬 확장 프로그램으로 브루 웹을 <em>직접 눌러</em> 영상 6편을 만들었어요.' },
        { t: 5, text: '사람이 한 건 프롬프트 한 줄. 그리고 <em>파일 저장 창</em>만 직접 눌렀어요.' },
        { t: 9.5, text: '어떻게 AI가 화면을 보고, 누르고, 글자를 칠 수 있었을까요?' }
      ],
      build({ stage, lines, P, tl }) {
        const q = P.quokka({ x: 70, y: 300, size: 380, pose: 'tablet' });
        stage.append(q.el);
        const b = P.bubble({ x: 330, y: 180, w: 470, text: '브루로 <b>영상 6편</b> 만들어 줘. 대본은 이 파일대로.', tail: 'left' });
        tl.at(stage.appendChild(b.el), .4, { from: 'left' });
        const screen = P.box({ x: 820, y: 150, w: 380, h: 420, label: '브루 (웹)', sub: '', accent: '' });
    ...
    },
    {
      ...
      captions: [
        { t: 0, text: '에이전트는 한 번에 답을 내지 않아요. <em>계획</em>하고, <em>도구</em>를 쓰고, <em>결과</em>를 보고, 다음 행동을 정해요.' },
        { t: 6, text: '이 고리를 목표에 닿을 때까지 <em>반복</em>해요. 실수를 해도 결과를 보고 고쳐요.' },
        { t: 11, text: '그래서 "에이전트"라고 불러요. 스스로 다음 걸음을 정하니까요.' }
      ],
      build({ stage, lines, P, tl }) {
    ...
    },
    {
      ...
      captions: [
        { t: 0, text: '도구는 모델이 부를 수 있는 <em>함수</em>예요. "화면 찍어 줘", "여기 눌러 줘", "이 글자 쳐 줘".' },
        { t: 5, text: '모델은 어떤 도구를 쓸지 <em>정하고</em>, 실제 실행은 앱(크롬 확장)이 해요.' },
        { t: 9.5, text: '실행 결과가 다시 모델에게 돌아가요. 그래서 다음 행동을 이어 갈 수 있어요.' }
      ],
      build({ stage, lines, P, tl }) {
    ...
    },
    {
      ...
      captions: [
        { t: 0, text: '도구가 많아지면 문제가 생겨요. AI 앱마다, 도구마다 <em>연결 방식이 제각각</em>이었거든요.' },
        { t: 4.5, text: 'MCP(Model Context Protocol)는 AI 앱과 도구를 잇는 <em>공개 표준 규격</em>이에요. 공식 문서는 "AI의 USB-C 단자"라고 불러요.' },
    ...
    },
    {
      ...
      captions: [
        { t: 0, text: '에이전트는 <em>권한을 준 범위</em> 안에서만 움직여요. 오늘 파일 저장 창은 보안상 <em>사람이 눌러야</em> 했어요.' },
        { t: 5.5, text: '그래서 일을 시킬 때는 <em>목표·범위·예산·멈출 조건</em>을 적어 줘요. 이 극장을 만든 프롬프트도 그랬어요.' },
        { t: 10.5, text: 'AI가 마음대로 조종하는 게 아니라, 우리가 정한 문 안에서 반복하는 거예요.' }
      ],
      build({ stage, lines, P, tl }) {
        ...
        const sub = P.text({ x: 290, y: 540, w: 900, text: '"브루로 6편 · 이 폴더만 · 크레딧 100 이하 · 저장 창이 뜨면 멈추기"', size: 24, weight: 600, cls: 'muted' });
        tl.at(stage.appendChild(sub.el), 8.2, { from: 'up' });
        const last = P.text({ x: 290, y: 610, w: 900, text: '우리가 정한 <i>문</i> 안에서, 목표에 닿을 때까지 반복해요.', size: 28, weight: 800 });
  ...
  interaction: {
    title: '에이전트 루프 시뮬레이터',
    desc: '목표를 고르고 <b>다음 단계</b>를 눌러 보세요. 계획 → 도구 → 관찰이 한 걸음씩 진행되고, 사람의 승인이 필요한 관문에서는 멈춰요. 예산(최대 단계 수)을 넘기면 스스로 멈춰요.',
    mount(el, P) {
      const GOALS = {
        vrew: {
          label: '브루로 영상 1편 만들기', budget: 8,
          steps: [
            { tool: '화면 보기', obs: '브루 첫 화면. "새로 만들기" 버튼이 보여요.' },
  ...
  teacherLines: [
    ...
    'AI가 뭔가를 대신 눌러 준다면, "어디까지 눌러도 되는지"를 먼저 정하는 게 <b>사람의 일</b>이에요.'
  ],
  tip: {
    body: '에이전트에게 일을 시킬 때는 <b>목표, 범위, 예산, 멈출 조건</b>을 적어 줘요. "이 폴더만", "크레딧 100 이하", "저장 창이 뜨면 멈추기"처럼요. 바로 이 극장을 만든 프롬프트가 그렇게 쓰여 있었어요.',
    extra: '되돌릴 수 없는 일(삭제, 발송, 결제)은 항상 사람이 마지막 버튼을 누르게 두세요.'
  },
  myth: {
    myth: 'AI가 컴퓨터를 마음대로 조종한다.',
    fact: '권한을 준 범위 안에서만 움직이고, 사람이 승인하는 지점이 있어요. 오늘도 파일 저장 창은 사람이 눌렀어요.'
  },
  sources: [
  ...
  script: `
오늘 아침에 있었던 일부터 얘기할게요. 제가 Claude한테 "브루로 영상 6편 만들어 줘"라고 한 줄 썼더니, 크롬 확장 프로그램으로 브루 웹을 직접 눌러서 6편을 다 만들었어요. 제가 손댄 건 파일 저장 창 하나였어요.

이게 되는 이유는 "에이전트" 구조 때문이에요. 한 번에 답을 내는 게 아니라, 계획하고, 도구를 쓰고, 결과를 보고, 다음 행동을 정하는 고리를 돌아요. 도구는 "화면 찍어 줘", "여기 눌러 줘" 같은 함수예요. 모델은 어떤 도구를 쓸지 정하고, 실제 실행은 앱이 해요.

그런데 도구가 많아지면 연결 방식이 제각각이라 곤란해져요. 그래서 나온 게 MCP, 공개 표준 연결 규격이에요. 공식 문서는 "AI의 USB-C"라고 불러요. 한 번 만든 도구를 여러 AI 앱에 꽂을 수 있죠.

기억할 건 하나예요. 에이전트는 우리가 준 권한 안에서만 움직이고, 되돌릴 수 없는 일은 사람이 눌러요. 그러니 시킬 때는 목표, 범위, 예산, 멈출 조건을 적어 주세요. 학생에게는 이렇게 말해 주세요. "AI 에이전트는 로봇 청소기 같아. 우리가 정한 방 안에서만 움직여."
`
};
