...
export default {
  ...
  summary: '영상 속 인물이 장면마다 다른 사람처럼 보였던 이유와, 레퍼런스 이미지로 얼굴을 붙잡아 두는 방법을 짚어봐요.',
  ...
  scenes: [
    ...
    },
    {
      ...
      captions: [
        ...
        { t: 5, text: '다만 도구마다 <em>이름과 사용법</em>이 달라요. 정확도 차이도 있고요.' },
      ...
      build({ stage, lines, P, tl }) {
        ...
        const chip = P.chip({ x: 400, y: 360, text: '정확도 차이도 있어요', color: 'orange', size: 22 });
        tl.at(stage.appendChild(chip.el), 5.4, { from: 'pop' });
  ...
  tip: {
    body: '도구별로 <b>캐릭터 고정</b> 또는 "레퍼런스 이미지" 기능 이름이 달라요. 정면 사진 1~3장을 넣고 같은 얼굴로 여러 장면을 만들어 보세요. 결과는 도구·설정마다 다르니 몇 번 다시 만들어 비교해 보는 게 좋아요.',
  ...
  script: `만든 영상에서 같은 인물인데 장면마다 얼굴이 조금씩 다르게 보였어요. 귀 크기도 털색도 장면마다 살짝 달랐죠. 왜 이럴까요?

이유는 간단해요. AI 이미지 생성 도구는 장면 하나하나를 완전히 새로 그려요. 매번 다른 시드에서 시작해 확률로 그림을 뽑다 보니, 같은 설명을 써도 세부가 달라져요.
...
요즘 영상 도구들도 '캐릭터 고정'이라는 이름으로 이 기능을 넣어 뒀어요. 다만 도구별로 정확도와 사용법이 달라요. 중요한 캐릭터는 정면 레퍼런스 이미지를 먼저 만들고, 장면마다 넣어 결과를 비교해 보세요.`,

  draftScenes: [
    '사례: 영상 속 인물이 장면마다 달라진 카드 3장을 나란히 보여주기',
    '왜 달라질까: 매 장면을 새로 그린다는 원리를 쿼카가 설명 (시드별 얼굴 카드가 순서대로 등장)',
    '레퍼런스 이미지 원리: 설명만 있을 때 vs 레퍼런스 이미지가 있을 때 비교, IP-Adapter·DreamBooth 한 줄 소개',
    "도구 소개: 요즘 영상 도구들의 '캐릭터 고정' 기능, 도구별로 정확도·사용법이 다르다는 점",
    '정리: 정면 레퍼런스 이미지를 먼저 만들고, 장면마다 넣고, 결과를 비교하기'
  ]
};
