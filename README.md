...
쿼카 해설자는 쿼카연구회 오리지널 쿼카를 바탕으로 AI로 생성한 이미지예요(포즈 8종, 편별 의상 12종, 첫 화면 히어로). 어떤 그림을 어떤 도구로 만들었는지는 [CREDITS.md](CREDITS.md)에 전부 적어 두었어요. 새 포즈를 추가하려면 [IMAGE_PROMPTS.md](IMAGE_PROMPTS.md)를 참고하세요.
